...
Aplicação web para checklist de sites de câmeras CFTV da Claro, cobrindo as **3 regionais** da operação (PR, SC, RS) com ~235 sites monitorados.
...
Suporte a planilhas das **3 regionais** (PR.html, SC.html, RS.html) exportadas do Google Sheets / Excel.

- **Modal completo** com zona de arrastar-e-soltar
- **Preview** dos dados antes de importar com badges "novo" / "atualizar"
...
- **Detecção automática** da regional pelo nome do arquivo (PR.html → PR, SC.html → SC, RS.html → RS)

### 🚀 Modo Ronda com Alertas Contextuais

- Interface focada, um site por vez, prioridade por criticidade (problemas primeiro)
- **Comparação de câmeras**: Ontem → Hoje → Última Ronda lado a lado
- **Alertas contextuais** exibidos ao abrir o site:
  - ⚠️ O.S. aberta
  - 🌿 Vegetação alta reportada (com data)
  - 🔴 Alarme desconectado há X dias
  - 📷 Câmera com problema registrado
  - 🔄 Problema recorrente (3+ das últimas 5 rondas com falha)
- Atalhos de teclado para máxima velocidade
...
| Atalho | Ação |
|--------|------|
| `Espaço` / `Enter` | Marcar OK |
| `P` | Marcar Parcial (abre modal de câmeras) |
| `O` | Marcar Offline/Desconectado |
| `S` | Pular site (reaparece no fim) |
...
| Regional | Estado | Sites |
|----------|--------|-------|
| PR | Paraná | ~86 sites |
| SC | Santa Catarina | ~83 sites |
| RS | Rio Grande do Sul | ~66 sites |
| **TOTAL** | **Regional Sul** | **~235 sites** |
...
2. Arraste os arquivos `PR.html`, `SC.html`, `RS.html` para a zona de drop, ou clique para selecionar
3. Revise o preview dos dados (badges **novo** / **atualizar** por site)
...
| SIGLA | Identificação (ex: PRLDA79) |
...
| B | CONTA | 1326 |
| C | SIGLA | PRLDA79 |
| D | STATUS alarme | ONLINE / DESCONECTADO |
| E | DATA desconexão | 09/03/26 |
| F | O.S. | ABERTA / ANULADA |
| G | ZONA | 1 |
| H | STATUS4 | MONITORADO |
| I | PADRÃO CÂMERAS | 4 |
| J | ONTEM | 3 |
| K | HOJE | 4 |
| L | STATUS CFTV | OK / PARCIAL / DESCONECTADO |
| M | DATA ALTERAÇÃO | 04/03/26 |
| N | VEGETAÇÃO ALTA | VERDADEIRO / FALSO |
| O–AC | Câmeras individuais | ⬛ escura, 🔶 obstruída, 📶 sem sinal, ❌ offline |
| AD | OBSERVAÇÃO | Texto livre |
...
- **[sql.js](https://github.com/sql-js/sql.js)** v1.10.3 — SQLite no navegador (sem servidor)
...
├── css/
│   ├── styles.css      # Design base (dark/light theme, ronda, comparação de câmeras)
│   ├── dashboard.css   # Estilos do dashboard (cards regionais, charts, tabela de problemas)
│   └── features.css    # KPIs, Timeline, Omnisearch, Modais, Relatórios, Importador HTML
├── js/
│   ├── app.js          # Controlador principal + Settings + Keyboard shortcuts
│   ├── database.js     # Wrapper SQLite (sql.js) com migrações
│   ├── xlsx-import.js  # Importação de planilhas XLSX com detecção de regional
│   ├── dataImporter.js # Importação HTML modal drag-and-drop + dados demo 235 sites
│   ├── dashboard.js    # Cards regionais, gráficos analíticos, tabela de problemas
│   ├── exporter.js     # Exportação Excel (multi-abas) / PDF / Clipboard
│   ├── ronda.js        # Modo ronda com alertas contextuais e comparação de câmeras
│   ├── sync.js         # Exportar/importar JSON multi-usuário
│   ├── kpi.js          # KPI cards + Timeline de atividades
│   ├── auditlog.js     # Audit log + Backup/Restore completo
